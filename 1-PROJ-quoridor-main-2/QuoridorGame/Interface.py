...
class Interface:
    def __init__(self, quoridor):
        ...
        # Wall Temporaire pour le placement des murs
        self.temporary_wall = None
            ...
            for event in pygame.event.get():
                ...
                # cheat/debug
                    ...
                    # debug add wall
                        ...
                        print("cheat mode on")
                        self.quoridor.add_wall(0, 2, 2, 2)
                        print(self.quoridor.walls)
                        self.quoridor.add_wall(1, 1, 1, 3)
                        print(self.quoridor.walls)
            self.draw()
        pygame.quit()
    ...
    def draw_walls(self):
        ...
        for wall in self.quoridor.walls:
            ...
            if x1 == x2 and abs(y2 - y1) == 2:
                x = self.board_rect.left + x1 * self.cell_size
                y = self.board_rect.top + min(y1, y2) * self.cell_size
                pygame.draw.line(self.screen, self.quoridor.current_player(
                ).color, (x, y), (x, y + self.cell_size * 2), 2)
            # Vérifie si le mur est horizontal et de longueur 2
            elif y1 == y2 and abs(x2 - x1) == 2:
                x = self.board_rect.left + min(x1, x2) * self.cell_size
                y = self.board_rect.top + y1 * self.cell_size
                pygame.draw.line(self.screen, self.quoridor.current_player(
                ).color, (x, y), (x + self.cell_size * 2, y), 2)

        # Dessine le mur temporaire
        if self.temporary_wall is not None:
            x1, y1, x2, y2 = self.temporary_wall
            if x1 == x2 and abs(y2 - y1) == 1:
                x = self.board_rect.left + x1 * self.cell_size + self.cell_size // 2
                y = self.board_rect.top + \
                    min(y1, y2) * self.cell_size + self.cell_size // 2
                pygame.draw.line(self.screen, self.quoridor.current_player(
                ).color, (x, y), (x, y + self.cell_size), 2)
